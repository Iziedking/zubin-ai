...
class RecursiveSolver:
    ...
    def __init__(
        ...
        Args:
            ...
            execution_id: Pre-assigned execution ID from API layer (optional)
        """
        self.config = config
    ...
    def __getstate__(self):
        ...
        state.pop('postgres_storage', None)
        state.pop('checkpoint_manager', None)
        state.pop('mlflow_manager', None)
        state.pop('observability', None)
        state.pop('runtime', None)
        state.pop('toolkit_manager', None)
    ...
    def _initialize_task_and_dag(
        ...
        if dag is None:
            dag = TaskDAG(execution_id=self.execution_id)
            self.last_dag = dag
